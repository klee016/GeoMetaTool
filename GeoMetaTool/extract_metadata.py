...
class MetadataExtractor:
    ...
    def __init__(self, rootDir, defaultValues, cswURL, username, password, upload=False):
        ...
        self.rootDate = utils.searchDate(os.path.basename(os.path.normpath(self.rootDir)))
        self.Catchwords = utils.searchCatchwords(os.path.basename(os.path.normpath(self.rootDir)))
    ...
    def createMetadata(self):
        ...
        for dirName, subdirList, fileList in os.walk(self.rootDir):
            ...
            Catchwords.extend(utils.searchCatchwords(os.path.basename(os.path.normpath(dirName))))
            #Extract possible Date information from directory
            dirdate = utils.searchDate(os.path.basename(os.path.normpath(dirName)))
            ...
            for fname in fileList:                                
                ...
                if fname.endswith(self.rasterExtensions + self.vectorExtensions):
                    ...
                    fileCatchwords = utils.searchCatchwords(os.path.basename(os.path.normpath(os.path.splitext(fname)[0])))
                    #Extract possible Date from file name
                    filedate = utils.searchDate(os.path.basename(os.path.normpath(fname)))
                    #If no Date could be found use rootDate
                    ...
                    if self.upload == True:
                        utils.CSWInsert(self.cswURL, self.username, self.password, xmlPath)
